...
public class TestCapSsfMan extends TesterBase implements TestCapSsfManMBean, Stoppable, CAPDialogListener,
    ...
    public String performInitialDp(String msg) {
        ...
        try {
            ...
            CallingPartyNumber callingPartyNumber = capProvider.getISUPParameterFactory().createCallingPartyNumber();
            callingPartyNumber.setAddress("593968582871");
            callingPartyNumber.setAddressRepresentationREstrictedIndicator(callingPartyNumber._APRI_RESTRICTED);
            callingPartyNumber.setNatureOfAddresIndicator(callingPartyNumber._NAI_INTERNATIONAL_NUMBER);
            callingPartyNumber.setNumberIncompleteIndicator(callingPartyNumber._NI_COMPLETE);
            callingPartyNumber.setNumberingPlanIndicator(callingPartyNumber._NPI_ISDN);
            callingPartyNumber.setScreeningIndicator(callingPartyNumber._SI_USER_PROVIDED_VERIFIED_PASSED);
            CallingPartyNumberCap callingPartyNumberCap =  capProvider.getCAPParameterFactory().createCallingPartyNumberCap(callingPartyNumber);
            CallingPartyCategory callingPartyCategory = capProvider.getISUPParameterFactory().createCallingPartyCategory();
            callingPartyCategory.setCallingPartyCategory((byte)10);
            CallingPartysCategoryInap callingPartysCategoryInap  = capProvider.getINAPParameterFactory().createCallingPartysCategoryInap(callingPartyCategory);
            ...
            LocationNumber locationNumber = capProvider.getISUPParameterFactory().createLocationNumber();
            locationNumber.setAddress("59397995033");
            locationNumber.setInternalNetworkNumberIndicator(locationNumber._INN_ROUTING_NOT_ALLOWED);
            locationNumber.setAddressRepresentationRestrictedIndicator(locationNumber._APRI_RESTRICTED);
            ...
            LocationNumberCap locationNumberCap = capProvider.getCAPParameterFactory().createLocationNumberCap(locationNumber);
            UserTeleserviceInformation highLayerCompatibility = capProvider.getISUPParameterFactory().createUserTeleserviceInformation();
            highLayerCompatibility.setCodingStandard(highLayerCompatibility._CODING_STANDARD_ITU_T);
            //highLayerCompatibility.setEHighLayerCharIdentification(highLayerCompatibility._h);
            //highLayerCompatibility.setEVideoTelephonyCharIdentification(highLayerCompatibility._vid);
            highLayerCompatibility.setHighLayerCharIdentification(highLayerCompatibility._HLCI_TELEPHONY);
            highLayerCompatibility.setInterpretation(highLayerCompatibility._INTERPRETATION_FHGCI);
            highLayerCompatibility.setPresentationMethod(highLayerCompatibility._PRESENTATION_METHOD_HLPP);
            HighLayerCompatibilityInap highLayerCompatibilityInap= capProvider.getINAPParameterFactory().createHighLayerCompatibilityInap(highLayerCompatibility);
            UserServiceInformation userServiceInformation = capProvider.getISUPParameterFactory().createUserServiceInformation();
            userServiceInformation.setCodingStandart(userServiceInformation._CS_CCITT);
            userServiceInformation.setInformationTransferCapability(userServiceInformation._ITS_SPEECH);
            userServiceInformation.setTransferMode(userServiceInformation._TM_CIRCUIT);
            userServiceInformation.setInformationTransferRate(userServiceInformation._ITR_64);
            //userServiceInformation.set(userServiceInformation._LAYER1_IDENTIFIER);
            userServiceInformation.setL1UserInformation(userServiceInformation._L1_G711_A);
            BearerCap bearerCap = capProvider.getCAPParameterFactory().createBearerCap(userServiceInformation);
            BearerCapability bearerCapability = capProvider.getCAPParameterFactory().createBearerCapability(bearerCap);
            IMSI imsi = capProvider.getMAPParameterFactory().createIMSI("740010154212150");
            ISDNAddressString vlrNumber = capProvider.getMAPParameterFactory().createISDNAddressString(AddressNature.international_number, NumberingPlan.ISDN, "59397999138");
            CellGlobalIdOrServiceAreaIdFixedLength cellGlobalIdOrServiceAreaIdFixedLength  = capProvider.getMAPParameterFactory().createCellGlobalIdOrServiceAreaIdFixedLength(740, 01, 50166, 64875);
            CellGlobalIdOrServiceAreaIdOrLAI cellGlobalIdOrServiceAreaIdOrLAI = capProvider.getMAPParameterFactory().createCellGlobalIdOrServiceAreaIdOrLAI(cellGlobalIdOrServiceAreaIdFixedLength);
            LocationInformation locationInformation = capProvider.getMAPParameterFactory().createLocationInformation(0, null, vlrNumber, null, cellGlobalIdOrServiceAreaIdOrLAI, null, null, null, null, false, false, null, null);
            ExtTeleserviceCode extTeleserviceCode  = capProvider.getMAPParameterFactory().createExtTeleserviceCode(TeleserviceCodeValue.telephony);
            ExtBasicServiceCode extBasicServiceCode = capProvider.getMAPParameterFactory().createExtBasicServiceCode(extTeleserviceCode);//"401c413903c1cc"
